...
`gtUtils` ships with a function that makes creating tier lists
...
All done! You’ve created a tier list in `gt` using `gtUtils`. As of
`gtUtils` v0.1, the tier list function is somewhat limited: a) it only
supports image cells and b) it will not “wrap” your logos to condense
the width (you can play around with image_height to do this).
